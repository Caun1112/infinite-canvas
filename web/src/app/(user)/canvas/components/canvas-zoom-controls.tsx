...
import { Focus, HelpCircle } from "lucide-react";
...
export function CanvasZoomControls({ scale, onScaleChange, onReset }: CanvasZoomControlsProps) {
    ...
    return (
        <div className="canvas-zoom-controls absolute bottom-5 left-5 z-50" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
            <div className="canvas-zoom-controls-panel flex h-14 items-center gap-1 rounded-xl border px-2 shadow-lg backdrop-blur" style={dockStyle}>
                <Tooltip title="重置视图">
                ...
                <span className="hidden w-10 text-right text-xs tabular-nums md:inline-block" style={{ color: theme.node.muted }}>
                    {Math.round(scale * 100)}%
                </span>
                <Tooltip title="快捷键">
                    <Button type="text" className="!h-8 !w-8 !min-w-8 !p-0" style={{ color: theme.toolbar.item }} icon={<HelpCircle className="size-4" />} onClick={() => setShortcutsOpen(true)} aria-label="快捷键" />
                </Tooltip>
            </div>
